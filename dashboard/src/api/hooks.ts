...
export const qk = {
  ...
  trajectories: ['reviews', 'trajectories'] as const,
  trajectory: (sessionId: string) => ['reviews', 'trajectories', sessionId] as const,
...
export const useTrajectories = () =>
  useQuery({ queryKey: qk.trajectories, queryFn: () => api.getTrajectories(50), refetchInterval: 10_000 })

export const useTrajectory = (sessionId: string) =>
  useQuery({ queryKey: qk.trajectory(sessionId), queryFn: () => api.getTrajectory(sessionId), enabled: !!sessionId })

export const useSaveTrajectoryReview = () => {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: ({ sessionId, body }: { sessionId: string; body: Parameters<typeof api.saveTrajectoryReview>[1] }) =>
      api.saveTrajectoryReview(sessionId, body),
    onSuccess: (_data, vars) => {
      qc.invalidateQueries({ queryKey: qk.trajectories })
      qc.invalidateQueries({ queryKey: qk.trajectory(vars.sessionId) })
    },
  })
}

export const useAssistTrajectoryReview = () =>
  useMutation({
    mutationFn: (sessionId: string) => api.assistTrajectoryReview(sessionId),
  })
