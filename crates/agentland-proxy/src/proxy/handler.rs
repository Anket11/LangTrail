...
pub async fn proxy_handler(
    ...
    if let Some(ref pool) = state.db_pool {
        match agentland_store::get_agent(pool, &agent_id).await {
            Ok(Some(agent_json)) => {
...
fn refine_streaming_from_body(protocol: Protocol, body: &Bytes) -> Protocol {
    match protocol {
        Protocol::OpenAI { version, .. } => {
            let streaming = agentland_common::protocols::openai::is_streaming(body);
            Protocol::OpenAI { version, streaming }
        }
        Protocol::Anthropic { version, .. } => {
            let streaming = agentland_common::protocols::anthropic::is_streaming(body);
...
fn detect_mcp_transport(headers: &http::HeaderMap) -> agentland_common::protocols::McpTransport {
    if headers
        ...
        .unwrap_or(false)
    {
        agentland_common::protocols::McpTransport::Sse
    } else {
        agentland_common::protocols::McpTransport::JsonRpc
    }
}
