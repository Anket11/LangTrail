...
pub fn create_router(state: Arc<AppState>) -> Router {
    Router::new()
        ...
        // ── Human trajectory reviews ──────────────────────────────────────
        .route(
            "/api/v1/reviews/trajectories",
            get(handlers::reviews::list_trajectories),
        )
        .route(
            "/api/v1/reviews/trajectories/{session_id}",
            get(handlers::reviews::get_trajectory).post(handlers::reviews::save_review),
        )
        .route(
            "/api/v1/reviews/trajectories/{session_id}/assist",
            post(handlers::reviews::assist_review),
        )
        .route("/api/v1/reviews/export", get(handlers::reviews::export_reviews))
        // ── Config ─────────────────────────────────────────────────────────
...
///
/// If `AGENTLAND_API_KEY` is set, all non-public paths require auth.
...
/// the Agentland REST API every 5 seconds. No build step or Node.js required.
...
async fn metrics_handler(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let body = format!(
        "# HELP agentland_requests_total Total proxy requests intercepted\n\
         # TYPE agentland_requests_total counter\n\
         agentland_requests_total {requests}\n\
         # HELP agentland_events_total Total events written to PostgreSQL\n\
         # TYPE agentland_events_total counter\n\
         agentland_events_total {events}\n\
         # HELP agentland_agents_active Distinct agents seen since process start\n\
         # TYPE agentland_agents_active gauge\n\
         agentland_agents_active {agents}\n"
    );
...
async fn metrics_stub_handler() -> impl IntoResponse {
    (
        ...
        "# HELP agentland_requests_total Total proxy requests intercepted\n\
         # TYPE agentland_requests_total counter\n\
         agentland_requests_total 0\n\
         # HELP agentland_events_total Total events written to PostgreSQL\n\
         # TYPE agentland_events_total counter\n\
         agentland_events_total 0\n\
         # HELP agentland_agents_active Distinct agents seen since process start\n\
         # TYPE agentland_agents_active gauge\n\
         agentland_agents_active 0\n",
    )
}
...
mod tests {
    #[test]
    fn dashboard_html_contains_agentland() {
        let html = include_str!("../../static/dashboard.html");
        assert!(html.contains("Agentland"));
