...
import type {
  ...
  TrajectorySummary, TrajectoryDetail, SaveTrajectoryReviewRequest,
...
// Reviews
export const getTrajectories = (limit = 25) =>
  request<PaginatedResponse<TrajectorySummary>>(`/api/v1/reviews/trajectories?limit=${limit}`)
export const getTrajectory = (sessionId: string) =>
  request<{ data: TrajectoryDetail }>(`/api/v1/reviews/trajectories/${sessionId}`)
export const saveTrajectoryReview = (sessionId: string, body: SaveTrajectoryReviewRequest) =>
  request<{ data: unknown }>(`/api/v1/reviews/trajectories/${sessionId}`, { method: 'POST', body: JSON.stringify(body) })
export const assistTrajectoryReview = (sessionId: string) =>
  request<{ data: ReviewAssistSuggestion }>(`/api/v1/reviews/trajectories/${sessionId}/assist`, { method: 'POST' })
export const exportTrajectoryReviews = async (): Promise<string> => {
  const res = await fetch('/api/v1/reviews/export', {
