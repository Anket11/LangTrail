//! Agentland proxy binary entry point.
//!
...
use agentland_common::config::Config;
use tracing_subscriber::EnvFilter;

use agentland_proxy::{api, events, proxy};
...
async fn main() -> anyhow::Result<()> {
    ...
    let pool_result = agentland_store::connect(&config.database).await;
    ...
    let policy_config_path = std::env::var("AGENTLAND_POLICY_FILE")
        .unwrap_or_else(|_| "config/policies.example.yaml".to_string());

    let policy_engine = {
        let path = std::path::Path::new(&policy_config_path);
        if path.exists() {
            match agentland_proxy::policy::PolicyEngine::from_file(path) {
                Ok(engine) => {
                ...
                Err(e) => {
                    ...
                    agentland_proxy::policy::PolicyEngine::noop()
                }
            }
        } else {
    ...
    let policy_hook: std::sync::Arc<dyn agentland_proxy::policy::PolicyHook> =
