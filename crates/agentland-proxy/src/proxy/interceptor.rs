...
use agentland_common::models::event::{AgentEvent, EventDirection, Provider};
use agentland_common::protocols::Protocol;
...
pub struct InterceptorState {
    ...
    pub db_pool: Option<agentland_store::StorePool>,
}
...
impl InterceptorState {
    ...
    pub fn with_pool_and_upstream_urls(
        ...
        policy_hook: Arc<dyn PolicyHook>,
        upstream_urls: UpstreamUrls,
        db_pool: agentland_store::StorePool,
    ) -> Self {
        Self {
            session_tracker: Mutex::new(SessionTracker::new()),
            event_sender,
...
pub async fn log_request_event(ctx: &RequestContext, state: &InterceptorState) {
    ...
    match &ctx.protocol {
        Protocol::OpenAI { .. } => {
            ...
            event.payload = summarize_json_payload("request", &ctx.request_body);
        }
        Protocol::Anthropic { .. } => {
            if let Some(req) =
                agentland_common::protocols::anthropic::parse_request(&ctx.request_body)
            {
                event.model = Some(req.model);
            }
            event.payload = summarize_json_payload("request", &ctx.request_body);
...
pub async fn log_response_event(
    ...
    match &ctx.protocol {
        Protocol::OpenAI { streaming, .. } => {
            if *streaming {
                ...
                agentland_common::protocols::openai::parse_response(response_body)
            {
        ...
        Protocol::Anthropic { streaming, .. } => {
            if *streaming {
                ...
                let streaming_acc =
                    agentland_common::protocols::anthropic::accumulate_streaming_events(
                        &acc_state.anthropic_events,
                    );
                event.model = streaming_acc.model.clone().or_else(|| {
                    agentland_common::protocols::anthropic::parse_model(&ctx.request_body)
                });
                ...
                agentland_common::protocols::anthropic::parse_response(response_body)
            {
                event.model = Some(resp.model);
    ...
    event.payload = summarize_json_payload("response", response_body);
...
pub async fn analyze_request(
    ...
    match protocol {
        Protocol::OpenAI { .. } => {
            let model = agentland_common::protocols::openai::parse_model(body);
            tracing::debug!(
                agent = agent_id,
                model = ?model,
                bytes = body.len(),
        ...
        Protocol::Anthropic { .. } => {
            let model = agentland_common::protocols::anthropic::parse_model(body);
            tracing::debug!(
                agent = agent_id,
                model = ?model,
                bytes = body.len(),
        ...
        Protocol::Mcp { server, .. } => {
            if let Some(req) = agentland_common::protocols::mcp::parse_request(body) {
                let tool = agentland_common::protocols::mcp::extract_tool_name(&req);
                tracing::debug!(
                    agent = agent_id,
                    server = server,
...
pub async fn analyze_response(
    ...
    match protocol {
        Protocol::OpenAI { .. } => {
            let _usage = agentland_common::protocols::openai::parse_usage(body);
        }
        Protocol::Anthropic { .. } => {
            let _usage = agentland_common::protocols::anthropic::parse_usage(body);
        }
        _ => {}
    }
...
fn summarize_json_payload(kind: &str, body: &Bytes) -> Option<serde_json::Value> {
    if body.is_empty() {
        return None;
    }

    match serde_json::from_slice::<serde_json::Value>(body) {
        Ok(value) => Some(serde_json::json!({ kind: value })),
        Err(_) => Some(serde_json::json!({
            kind: String::from_utf8_lossy(body).chars().take(4_000).collect::<String>(),
        })),
    }
}
